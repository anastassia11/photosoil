        ...
        <div className="sm:space-y-6 sm:max-w-md m-auto sm:mt-16 w-full">
            <div className="text-center">
                <div className="mt-5 space-y-2">
                    ...
                    {!submitting && <p className="">{t('join_text')}<br />
                        {t('account_exists')} <Link
                            className='text-blue-600 hover:underline duration-300'
                            href={`/${locale}/login`}>{t('login')}</Link></p>}
                </div>
            </div>
            {!submitting ? <div className="sm:bg-white sm:shadow p-4 py-6 sm:p-6 sm:rounded-lg">
                <form onSubmit={handleSubmit(handleFormSubmit)}
                    ...
                    />
                    <SubmitBtn isSubmitting={isSubmitting} btnText={t('submit')} />
                </form>
            </div> : <p className='text-center'>Ваша заявка принята! Скоро мы с Вами свяжемся.</p>}
        </div>
    )
}
