...
export default function CreateNewsComponent() {
	...
	const handleSubmit = async ({ createTwoLang, isEng, news, newsPhotos }) => {
		try {
			const langNews = {
				...news,
				translations: news.translations.filter(
					({ isEnglish }) => isEnglish === isEng
				)
			}
			if (createTwoLang) {
				newsPhotos.map(photo =>
					editPhoto(photo.id, {
				...
				if (isEng) {
					newsPhotos.map(photo =>
						editPhoto(photo.id, { titleEng: photo.titleEng || '' })
					)
				} else {
					newsPhotos.map(photo =>
						editPhoto(photo.id, { titleRu: photo.titleRu || '' })
					)
				}
			}
	...
	return (
		<NewsForm
			onNewsSubmit={handleSubmit}
			btnText={t('create_news')}
			title={t('creation_news')}
		/>
	)
}
