...
export default function ObjectsPopup({ visible, objects, onCloseClick }) {
					...
					<div className='flex flex-row'>
						{object?.photo?.path ? (
							<div className='max-w-[40%] w-[40%]'>
								<Image
									src={`${BASE_SERVER_URL}${object.photo?.path}`}
			...
			<div className='flex flex-col'>
				...
				>
					<svg
						...
						viewBox='0 0 20 20'
						fill='currentColor'
						className={`mr-1 size-6 duration-150 ${isVisible[type] ? 'rotate-180' : ''}`}
					>
						<path
							fillRule='evenodd'
				...
				{
					<ul
						className={`flex flex-col duration-300 transition-all overflow-hidden ${isVisible[type] ? 'max-h-fit opacity-100 visible translate-y-0' : '-translate-y-2 max-h-0 invisible opacity-0'}`}
					>
						{type === 'soils'
							? soils.map(obj => <li key={obj.id}>{ObjectCard(obj)}</li>)
							: type === 'ecosystems'
								? ecosystems.map(obj => <li key={obj.id}>{ObjectCard(obj)}</li>)
								: publications.map(obj => (
										<li key={obj.id}>{ObjectCard(obj)}</li>
									))}
					</ul>
				}
			</div>
	...
	return (
		<div
			className={`${visible ? '-left-[0px] z-30' : 'sm:-left-[440px] z-20 -left-[calc(100%-90px)]'} 
     ...
     sm:max-h-[calc(100%-16px)] max-h-[calc(100%-100px)] 
		...
		>
			<div className={`relative flex-1 flex flex-col max-w-full`}>
				...
				{objects.length ? (
					<div className='flex flex-col sm:py-4 py-2 space-y-2 max-h-full overflow-y-auto scroll'>
