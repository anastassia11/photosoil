...
export default function TextEditor({ content, setContent }) {
  ...
  const handleHeadingChange = (level) => {
    level != 0 ? editor.chain().focus().toggleHeading({ level: Number(level) }).run() : editor.chain().focus().setParagraph().run()
  }
    ...
    <div className={`cursor-text min-h-[300px] editor mt-1 p-2 bg-white border rounded-md focus:border-blue-600 ${isFocusing ? 'border-blue-600' : ''}`}
      ...
      <div className="flex flex-row flex-wrap space-x-4 border-b pb-2 mb-2">
        ...
        <BubbleMenu
          ...
          <a href={editor.getAttributes("link").href} className='max-w-full text-nowrap text-ellipsis overflow-hidden text-blue-600 underline underline-offset-2 duration-300'>
        ...
        <div className='flex flex-row'>
          ...
          rounded-md hover:text-blue-600 duration-300`}
            ...
            <Icons.Underline strokeWidth={editor.isActive('underline') ? '2.5' : '2'} />
          </button>
        </div>
