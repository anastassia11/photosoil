	...
	function Dropdown({
		...
		useEffect(() => {
			if (value !== null && value !== undefined)
				setSelectedValue({ name: items[value], id: value })
		}, [value, items])
		...
		return (
			<div
				className={`${flexRow && 'flex flex-row items-center space-x-3 '} backface`}
			...
			>
				...
				<div className={`${flexRow ? 'w-full' : 'mt-1'} relative`}>
					<div
						className={`h-[40px] ${dropdownKey} ${!isTransparent && 'bg-white border'} flex cursor-pointer 
						...
						onClick={() =>
							dispatch(
								setDropdown({
									key: dropdownKey,
									isActive:
										dropdown.key !== null && dropdown.key !== dropdownKey
											? true
											: !dropdown.isActive
								})
							)
					...
					>
						...
						<span
							className={`transition ${dropdown.key == dropdownKey && dropdown.isActive ? '-rotate-180' : ''} `}
						>
							<svg
					...
					<div
						className={`z-50 duration-200 transition-all 
