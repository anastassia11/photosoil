...
export default function FileCard({ name, fileName, path, isLoading, onDelete }) {
    const fileTitle = <div className='mt-1 flex flex-row min-w-0'>
        {/* <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="-4 0 40 40"
            className='min-w-5 min-h-5 w-5 h-5 mr-1' >
            <path
                fill="#EB5757"
    ...
    const loadingTitle = <div className='mt-1 flex flex-row items-center justify-start'>
        <Oval
            height={20}
            width={20}
            color="#111827"
    ...
    return <div className='flex flex-row overflow-hidden justify-start space-x-2 items-center'>
        ...
        {!isLoading ? <button type='button' className='text-zinc-500 hover:text-zinc-800 duration-300 pt-[1px] ml-1'
            onClick={onDelete}>
            <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className='min-w-[10px] min-h-[10px] w-[10px] h-[10px]'>
                <g id="Menu / Close_LG">
