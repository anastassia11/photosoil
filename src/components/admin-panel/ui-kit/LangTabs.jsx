...
export default function LangTabs({ onLangChange, onTwoLangChange, isEng, oldIsEng, createTwoLang, oldTwoLang, isEdit }) {
    ...
    return (
        <Tabs.Root defaultValue={false} className="pt-2 md:col-span-2 sticky top-0 z-40  bg-[#f6f7f9]" value={isEng}
            ...
            <Tabs.List className="w-full border-b flex md:items-center gap-x-4 overflow-x-auto justify-between md:flex-row flex-col">
                ...
                {(!oldTwoLang || !isEdit) && <label htmlFor='createTwoLang' className={`md:order-2 order-1 pb-4 pr-1 md:pb-2.5 flex flex-row cursor-pointer items-center`}>
                    ...
                    <span>{isEdit ? `${oldIsEng ? t('add_ru') : t('add_en')}` : t('create_two_lang')}</span>
                </label>}
            </Tabs.List>
        </Tabs.Root>
    )
