...
export default function PublicationForm({ _publication, pathname, onPublicationSubmit, isLoading, btnText, oldTwoLang, oldIsEng }) {
    ...
    const handleInputChange = (e) => {
        ...
        setPublication(prevPubl => {
            const _prevPubl = name === 'doi' ? { ...publication, [name]: value } : {
                ...publication, translations: publication.translations.map(translation =>
    ...
    const handleCoordChange = (e, index) => {
        const { name, value } = e.target;
        setCoordinates(prev => prev.map((item, idx) => idx === index ? item.map(coord =>
            name === 'longtitude' ? [value, coord[1]] : [coord[0], value]
        ) : item));
    }

    const handleDeleteCoord = (index) => {
        const updatedArray = [...coordinates];
        updatedArray.splice(index, 1);
        console.log(coordinates)
        console.log(index)
        setCoordinates(updatedArray);
    }
    ...
    const ArrayInput = () => {
        return <div className='flex flex-col w-full '>
            <ul>
                {coordinates?.map((item, index) => <li className='flex flex-row mb-1' key={`coord_${index}`}>
                    <input
                        value={coordinates[index][1] || ''}
                        onChange={(e) => handleCoordChange(e, index)}
                        name='latitude'
                        type="text"
                    ...
                    <input
                        ...
                        className="ml-2 bg-white w-full mt-1 p-2 outline-none border focus:border-blue-600 shadow-sm rounded-md"
                    />
                    <button type='button'
                        className='p-2'
    ...
    return (
        <form
                ...
                <div className='grid grid-cols-2 gap-4 w-full'>
                    ...
                    <div className='flex flex-col w-full'>
                        ...
                        <div id='map-section' className='border rounded-lg overflow-hidden mt-2 min-h-[574px]'>
                            <MapArraySelect coordinates={coordinates}
            ...
            <button
                onClick={() => onPublicationSubmit({ createTwoLang, isEng, publication })}
                disabled={isLoading}
