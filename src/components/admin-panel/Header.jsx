...
export default function Header() {
	...
	return (
		<div className='relative w-fit self-end flex flex-row justify-center items-center'>
			<div
				className='user relative w-fit self-end flex flex-row items-center space-x-4
				...
				onClick={() =>
					dispatch(
						setDropdown({
							key: 'user',
							isActive:
								dropdown.key !== null && dropdown.key !== 'user'
									? true
									: !dropdown.isActive
						})
					)
			...
			>
				...
				<div className='flex flex-col justify-center items-start'>
					<p
						className={`font-semibold ${dropdown.key == 'user' && dropdown.isActive ? 'text-blue-700' : ''} duration-300`}
					>
						{email}
					</p>
				...
				<span
					className={`transition ${dropdown.key == 'user' && dropdown.isActive ? '-rotate-180' : ''} `}
				>
					<svg
					...
					>
						{!isLoading && (
							<svg
								...
								strokeWidth={1.5}
								stroke='currentColor'
								className='w-5 h-5 -translate-x-[3px]'
							>
								<path
									strokeLinecap='round'
									strokeLinejoin='round'
