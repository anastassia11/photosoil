...
export default function EditObject({ id, type, title }) {
	...
	useEffect(() => {
		if (typeof document !== 'undefined' && Object.keys(object).length) {
			const title = object.translations?.find(
	...
	const fetchObject = async () => {
		...
		if (result.success) {
			...
			if (type === 'soil') {
				...
				dataForPut.soilTerms = data.classification
					?.map(item => item.terms.map(({ id }) => id))
					.flat()
	...
	const editObject = async data => {
		const result =
			type === 'soil'
				? await putSoil(data.id, data)
				: await putEcosystem(data.id, data)
		if (result.success) {
		}
	}
	...
	const submitForm = async data => {
		...
		try {
			await Promise.all([
				...
				...objectPhoto.map(photo =>
					editPhoto(
						photo.id,
						createTwoLang
							? { titleEng: photo.titleEng || '', titleRu: photo.titleRu || '' }
							: currentLang
			...
			router.push(
				`/${locale}/admin/${type === 'soil' ? 'objects' : 'ecosystems'}`
			)
